...
sys.path.append('C:\\Users\\\tamot\\AppData\\Local\\Microsoft\\WindowsApps\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\python3.9')
sys.path.append('C:\\Users\\tamot\\AppData\\Local\\Microsoft\\WindowsApps\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\python3')
sys.path.append('C:\\Users\\tamot\\AppData\\Local\\Packages\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\LocalCache\\local-packages\\Python39\\site-packages')
sys.path.append('C:\\Users\\tamot\\AppData\\Local\\Packages\\PythonSoftwareFoundation.Python.3.9_qbz5n2kfra8p0\\LocalCache\\local-packages\\Python39\\dist-packages')
...
def VisionAndPlanner(GoalLocation,DistToGoalOne): #DO WORK HERE
    ...
    if(D[200,120] < .35): #regular empty space is around D(x,y) = ~.37
        VelX = 1
    if(D[60,120] <.35 or D[180,120] <.35):
        VelX = -1.5
    else:
        velX = -0
    return VelX 
...
def Controller(TrafficCylinder, Camera):
    ...
    while(not (GoalReached or OutOfBounds or bpy.data.scenes['Scene'].frame_current>=MaxFrames)): 
        Render()   
        DistToGoalOne = math.sqrt((GoalLocation[0]-Camera.location[0])**2 + (GoalLocation[1]-Camera.location[1])**2 + (GoalLocation[2]-Camera.location[2])**2)
